...
The supplied `.env.example` sets `SMTP_HOST=smtp.office365.com`, `SMTP_PORT=587`, `UPSTREAM_TLS_SECURITY=verify`, `UPSTREAM_SASL_ENABLE=yes`, and `SMTP_INET_PROTOCOLS=ipv4`. It also explicitly assigns `DNS_SERVER_1` and `DNS_SERVER_2` to the Postfix container, bypassing Docker's embedded resolver. The startup script copies that resolver configuration into `/var/spool/postfix/etc/resolv.conf`, because Debian runs the outbound Postfix `smtp(8)` daemon in a chroot. This is required when `docker compose exec postfix getent ahostsv4 smtp.office365.com` succeeds but Postfix logs `Name service error ... type=A: Host not found`. Replace the defaults with reachable organizational resolvers if required. Set `SMTP_INET_PROTOCOLS=all` only when IPv6 is known to work. The relay refuses to start if `secrets/sasl_passwd` is missing or empty.
