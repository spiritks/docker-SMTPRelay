...
The supplied `.env.example` sets `SMTP_HOST=smtp.office365.com`, `SMTP_PORT=587`, `UPSTREAM_TLS_SECURITY=verify`, `UPSTREAM_SASL_ENABLE=yes`, and `SMTP_INET_PROTOCOLS=ipv4`. It also explicitly assigns `DNS_SERVER_1` and `DNS_SERVER_2` to the Postfix container, bypassing Docker's embedded resolver. This is required when the host resolves `smtp.office365.com` but Postfix logs `Name service error ... type=A: Host not found`. Replace the defaults with reachable organizational resolvers if required. Set `SMTP_INET_PROTOCOLS=all` only when IPv6 is known to work. The relay refuses to start if `secrets/sasl_passwd` is missing or empty.
